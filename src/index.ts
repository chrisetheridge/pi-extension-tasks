...
import {
  ...
  type ThemeLike as PanelThemeLike,
} from "./task-panel.ts";
...
function taskByInput(
  ...
  return store
    ...
    .find(
      (task) =>
        task.id === candidate.id || task.title.toLowerCase() === candidate.title.toLowerCase(),
    );
}
...
function applySnapshot(runtime: RuntimeState): string {
  ...
  return tasks
    .map(
      (task) => `${task.id}: ${task.title} [${task.status}${task.owner ? ` @${task.owner}` : ""}]`,
    )
    .join("\n");
}
...
function getPanelActions(ctx: ExtensionContext, runtime: RuntimeState): TaskPanelActions {
  return {
    ...
    complete: async (task) => {
      ...
      await writeTask(
        config.tasksDir,
        mergeTaskPatch(task, { status: "complete", owner: undefined }),
      );
      await refreshRuntime(ctx, runtime);
    },
    delete: async (task) => {
...
function openPanel(ctx: ExtensionContext, runtime: RuntimeState): void {
  ...
  const promise = ctx.ui.custom<void>((tui, theme, _keybindings, done) => {
...
export default function taskOverlayExtension(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    promptSnippet:
      "Use the tasks tool to keep markdown-backed task files synchronized with the live task panel.",
    promptGuidelines: [
