...
function taskSearchText(task: TaskItem): string {
	return [task.id, task.title, task.status, task.owner, task.body, task.notes].filter(Boolean).join(" ");
}
...
export class TaskPanel implements Component {
	...
	private handleListInput(data: string): void {
		...
		if (isUp(data)) {
			if (tasks.length > 0) this.selectedIndex = this.selectedIndex === 0 ? tasks.length - 1 : this.selectedIndex - 1;
			this.requestRender();
			return;
		}
		if (isDown(data)) {
			if (tasks.length > 0) this.selectedIndex = this.selectedIndex === tasks.length - 1 ? 0 : this.selectedIndex + 1;
			this.requestRender();
			return;
		}
		if (isEnter(data)) {
	...
	private renderList(width: number): string[] {
		...
		const lines = [
			this.styleHeader(truncateToWidth(`Tasks (${tasks.length} total, ${progress} progress, ${complete} complete)`, width)),
		...
		if (tasks.length === 0) {
			lines.push("");
			lines.push(this.theme.fg("muted", truncateToWidth("No tasks yet. Sync tasks to create markdown task files.", width)));
			lines.push(this.theme.fg("dim", truncateToWidth("Enter text after /tasks or use the tasks tool.", width)));
			return lines;
		}
		...
		lines.push("");
		const maxVisible = 10;
		const start = Math.max(0, Math.min(this.selectedIndex - Math.floor(maxVisible / 2), filtered.length - maxVisible));
		...
		lines.push("");
		lines.push(this.theme.fg("dim", truncateToWidth("Type to search - arrows select - Enter actions - Esc close", width)));
		return lines;
	}
	...
	private renderActions(width: number): string[] {
		...
		for (const [index, action] of ACTIONS.entries()) {
			...
			lines.push(index === this.actionIndex ? this.theme.fg("accent", truncateToWidth(text, width)) : truncateToWidth(text, width));
		}
		return lines;
	}
	...
	private renderDetail(width: number): string[] {
		...
		const meta = this.theme.fg(
			"dim",
			truncateToWidth(`${task.id} - ${statusLabel(task.status)}${task.owner ? ` - ${task.owner}` : ""}`, width),
		);
